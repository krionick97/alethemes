...
function wayup_get_breadcrumbs() {
  // $text['home']   = 'Главная'; // - для фриланса и небольших заказов

  $text['home']     = __('Home','wayup'); // - более профессиональный подход, для коммерческих шаблонов
  ...
  if (is_home() || is_front_page()) {
      ...
      if ( is_category() ) {
          ...
          $post_type = get_post_type_object(get_post_type());
          if($post) { echo $before . $post_type->labels->singular_name . $after; }
          else { echo $before . wp_title($sep = '') .$after; }
